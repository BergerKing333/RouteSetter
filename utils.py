    ...
    for i in range(holdArray.size):
        if int(holdArray[i]) != 0:
            ...
            try:
                ...
                print("Tried to draw on an invalid hold")
    if saveImg:
...
def translateCoords(HOLDCOORDINATES):
...
def drawMultipleImages(holdArrays, holdCoordinates=HOLDCOORDINATES, saveImg=True, name="output.png"):
    xCoords, yCoords = translateCoords(holdCoordinates)
    img = np.array([])
    
    if holdArrays.shape[0] != 4:
        print("Invalid number of holdArrays")
        return
    routes = []
    for holdArray in holdArrays:
        routes.append(drawImg(holdArray=holdArray, holdCoordinates=holdCoordinates, saveImg=False))

    top_row = np.hstack((routes[0], routes[1]))
    bottom_row = np.hstack((routes[2], routes[3]))
    img = np.vstack((top_row, bottom_row))

    if saveImg:
        img = cv2.resize(img, (1400, 1400), interpolation=cv2.INTER_AREA)
        cv2.imwrite(name, img)
    return img
