import requests
import json
from utils import translateCoords
...
class webscraper:
    def __init__(self, angle = 45, numResults = 10, showCampusOnly = False) -> None:
        self.angle = angle
        ...
        self.xCoords, self.yCoords = translateCoords(self.HOLDCOORDINATES)
    ...
    def writeCSV(self):
        ...
        # new way of generating CSVs with formatting
        for route in data['climbs']:
            img = np.zeros((len(self.xCoords), len(self.yCoords)), dtype = int)
            for hold in route['holds']:
                index = self.HOLD_ID.index(int(hold))
                holdCoords = self.HOLDCOORDINATES[index]
                img[self.yCoords.index(holdCoords[1])][self.xCoords.index(holdCoords[0])] = int(route['holds'][hold])
            routeList.append(img.flatten())
            ratingList.append(int(route[f'vgrade{self.angle}'][1:]))
            # plt.imshow(img)
            # plt.show()
        shape_row = pd.DataFrame([(len(self.yCoords), len(self.xCoords))], columns=['shape_y', 'shape_x'])
        shape_row.to_csv(f'training_data{self.angle}.csv', mode='w')
        df = pd.DataFrame(routeList, columns=[f'hold{i}' for i in range(len(routeList[0]))])
        df = pd.concat([df, pd.DataFrame(ratingList, columns=['rating'])], axis=1)
        print(f"Writing {self.numResults} routes to training_data{self.angle}.csv")
        df.to_csv(f'training_data{self.angle}.csv', mode='a')


        # Old way of generating CSVs w/o reshape
        # for route in data['climbs']:
