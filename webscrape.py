...
class webscraper:
    def __init__(self, angle = 30, numResults = 2, showCampusOnly = False) -> None:
        self.angle = angle
        ...
        self.xCoords, self.yCoords = self.translateCoords(self.HOLDCOORDINATES)
    ...
    def writeCSV(self):
        ...
        print("Writing {self.numResults} routes to training_data{self.angle}.csv")
    ...
    def translateCoords(self, HOLDCOORDINATES):
        xCoords = [coord[0] for coord in HOLDCOORDINATES]
        yCoords = [coord[1] for coord in HOLDCOORDINATES]
        setx = set(xCoords)
        sety = set(yCoords)
        return (sorted(list(setx)), sorted(list(sety)))
